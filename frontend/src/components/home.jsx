...
const Home = () => {
    ...
    useEffect(()=>{
        ...
        fire.firestore().collection("folders").where("parent" , "==" , name ).get().then(async (folders)=>{
            const folderData = await folders.docs.map((folder)=>folder.data());
            setFoldersName(folderData);
        });
        fire.firestore().collection("files").where("parent" , "==" , name).get().then(async (files)=>{
            const fileData = await files.docs.map((file)=>file.data());
            setFilesName(fileData);
            console.log(filesName)
        })
        fire.firestore().collection("uploads").where("parent" , "==" , name).get().then(async (files)=>{
            const uploadData = await files.docs.map((file)=>file.data());
            setUploadFilesName(uploadData);
